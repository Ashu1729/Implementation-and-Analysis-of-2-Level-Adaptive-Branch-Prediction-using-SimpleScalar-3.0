...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_string(odb, "-bpred",
		 "branch predictor type {nottaken|taken|bimod|2lev|2levconf|comb}",
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  if (pred)
    bpred_reg_stats(pred, sdb);
      extern counter_t bpred_conf_updates;
  extern counter_t bpred_conf_correct;

  stat_reg_counter(sdb, "bpred_conf_updates",
                   "total number of confidence updates",
                   &bpred_conf_updates, 0, NULL);
  stat_reg_counter(sdb, "bpred_conf_correct",
                   "confidence-based correct predictions",
                   &bpred_conf_correct, 0, NULL);
  stat_reg_formula(sdb, "bpred_conf_accuracy",
                   "confidence prediction accuracy",
                   "bpred_conf_correct / bpred_conf_updates", NULL);
