...
/* --- Confidence predictor statistics --- */
unsigned int bpred_conf_updates = 0;
unsigned int bpred_conf_correct = 0;
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  switch (class) {
    case BPred2Level:
    {
      ...
      /* allocate first-level shift registers */
      ...
      /* allocate 2nd-level prediction table */
      ...
      /* allocate confidence and last_outcome tables safely */
      pred_dir->config.two.confidence = calloc(l2size, sizeof(unsigned char));
      if (!pred_dir->config.two.confidence)
        fatal("cannot allocate confidence table");

      pred_dir->config.two.last_outcome = calloc(l2size, sizeof(unsigned char));
      if (!pred_dir->config.two.last_outcome)
        fatal("cannot allocate last_outcome table");

      /* initialize prediction table entries to weakly taken/not-taken alternately */
      flipflop = 1;
      for (cnt = 0; cnt < l2size; cnt++)
      {
        pred_dir->config.two.l2table[cnt] = flipflop;
        flipflop = 3 - flipflop;
        pred_dir->config.two.confidence[cnt] = 0;    /* NEW: start at 0 confidence */
        pred_dir->config.two.last_outcome[cnt] = 0;  /* NEW: initialize to 'not taken' */
      }
					 ...
					 * used on mispredict recovery */
{
  ...
  if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
  {
    ...
    // --- Confidence-based prediction access ---
    struct bpred_dir_t *dpred = pred->dirpred.twolev;
    unsigned index = ((baddr >> MD_BR_SHIFT) ^ dpred->config.two.shiftregs[0]) & (dpred->config.two.l2size - 1);

    // Current confidence value for this entry
    unsigned char conf = dpred->config.two.confidence[index];

    // You can print or log confidence to check during debugging
    // fprintf(stderr, "Confidence[%u] = %d\n", index, conf);

    // Optional: modify prediction behavior based on confidence
    // Example: if confidence < threshold, force not taken
    if (conf < 2) {
      *(dir_update_ptr->pdir1) = 1;  // weakly not taken
    }
  }
  break;
  ...
  pbtb = &pred->btb.btb_data[index * pred->btb.assoc];

  /* search BTB for matching branch address */
  for (i = 0; i < pred->btb.assoc; i++) {
    if (pbtb->addr == baddr) {
      /* BTB hit */
      if (pbtb->target == btarget)
        pred->addr_hits++;
      else
        pred->misses++;
      return pbtb->target;
  ...
  /* BTB miss, return not-taken address */
  pred->misses++;
  return baddr + sizeof(md_inst_t);
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  /* update predictor ... */
  if (pred->dirpred.twolev &&
      (pred->class == BPred2Level ||
       (pred->class == BPredComb && dir_update_ptr->dir.twolev)))
  {
    struct bpred_dir_t *dp = pred->dirpred.twolev;
    unsigned l1index = (baddr >> MD_BR_SHIFT) & (dp->config.two.l1size - 1);
    unsigned l2index = ((baddr >> MD_BR_SHIFT) ^ dp->config.two.shiftregs[l1index])
                       & (dp->config.two.l2size - 1);

    /* --- Confidence and last-outcome update --- */
    if (dp->config.two.last_outcome && dp->config.two.confidence)
    {
      unsigned char actual = taken ? 1 : 0;

      if (dp->config.two.last_outcome[l2index] == actual)
      {
        if (dp->config.two.confidence[l2index] < 255)
          dp->config.two.confidence[l2index]++;   /* increase confidence */
      }
      else
        dp->config.two.confidence[l2index] = 1;   /* reset confidence */

      dp->config.two.last_outcome[l2index] = actual;  /* store latest outcome */
      if (!correct)
    dp->config.two.confidence[l2index] = 0;
    }

    /* --- Reinforce predictor if highly confident --- */
    const unsigned CONF_THRESHOLD = 6;
    if (dp->config.two.confidence &&
        dp->config.two.confidence[l2index] >= 3)
    {
      if (dir_update_ptr->pdir1)
      {
        if (taken)
        {
          if (*dir_update_ptr->pdir1 < 3)
            ++*dir_update_ptr->pdir1;  /* push toward taken */
        }
        else
        {
          if (*dir_update_ptr->pdir1 > 0)
            --*dir_update_ptr->pdir1;  /* push toward not-taken */
        }
      }
    }
    }
  ...
  if (dir_update_ptr->pmeta)
      ...
      if (dir_update_ptr->dir.bimod != dir_update_ptr->dir.twolev)
	{
      ...
      /* >>> BEGIN: 2-level confidence & last-outcome tracking (STEP-4) */
  /* We track confidence/last_outcome for the 2-level predictor entries.
     This code updates confidence and optionally applies a stronger update
     to the 2-level counter when confidence is high. */

  /* Only update these arrays when the 2-level predictor was involved:
     - pred->class == BPred2Level  OR
     - pred->class == BPredComb && dir_update_ptr->dir.twolev == 1
  */
    /* >>> BEGIN: 2-level confidence & last-outcome tracking (STEP-4) */
  /* We track confidence/last_outcome for the 2-level predictor entries.
     This code updates confidence and optionally applies a stronger update
  ...
  */
  ...
  /* >>> END: 2-level confidence & last-outcome tracking (STEP-4) */
